...
function getUsersFromDatabase(array) {
	...
	const arr = Array.from(
		userRecords,
		({ id, name, age }) => ({ id, firstName: name.toUpperCase(), years: age })
	);
	return arr;
}
...
function getElementAtIndex(arr, index) {
	...
	if (arr.length === 0) { return undefined }
	else if (index <= arr.length) { return arr.at(index) }
...
function combineAndReverseArrays(arr1, arr2) {
	...
	// Якщо хоча б один з аргументів не є масивом, виводимо повідомлення про помилку
	// Повертаємо пустий масив
	if (Array.isArray(arr1) && Array.isArray(arr2)) {
		return arr1.concat(arr2).reverse();
	}
	else {
		console.log("Помилка: Один або обидва аргументи не є масивами.");
		return [];
	}
...
function findElementIndexes(arr, element) {
	...
	// Якщо елемент не знайдено, повертаємо пустий масив
